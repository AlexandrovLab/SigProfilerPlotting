...
def plotSV(matrix_path, output_path, project, plot_type="pdf", percentage=False):
    ...
    def plot(counts, labels, sample, project, percentage):
...
def plotCNV(matrix_path, output_path, project, plot_type="pdf", percentage=False):
    ...
    def plot(counts, labels, sample, project, percentage):
        ...
        # else:
        #  ax.set_title(sample, loc='left', y=1.3)
    ...
    plt.rcParams['axes.facecolor'] = 'white'
    df = pd.read_csv(matrix_path, sep=None, engine='python') #flexible reading of tsv or csv
    # #print('Done reading in matrix')
    # if aggregate:
    #   pp = PdfPages(output_path + project + '_CNV_aggregated_counts.pdf') #counts for a grouping of samples
    # else:
    if plot_type == 'pdf':
        pp = PdfPages(output_path + project + '_CNV_signatures' + '.pdf')
    else: #input == counts
        print("The only plot type supported at this time is pdf")
            #pp = PdfPages(output_path + project + '_CNV_counts' + '.pdf') #counts for EACH sample

    #each column vector in dataframe contains counts for a specific sample
    label = df.columns[0]
    labels = df[label]
    samples = list(df)[1:]
    #one plot for each sample(or signature) all combined in a single pdf
    for i, (col, sample) in enumerate(zip(df.columns[1:], samples)):
        counts = list(df[col])
        plot(counts, labels, sample, project, percentage)
