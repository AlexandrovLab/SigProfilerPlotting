...
def plotCNV(matrix_path, output_path, project, plot_type="pdf", percentage=False, aggregate=False, write_to_file=True):
    ...
    def plot(counts, labels, sample, project, percentage, aggregate=False, write_to_file=True):
        ...
        ax.spines["left"].set_color("black")
        # Add light gray horizontal lines at y-ticks
        ax.grid(linestyle='-', linewidth=1, color='#EDEDED')
        plt.xlim(xmin=-.5,xmax=len(labels)-.5)
        tmp_max=max(counts)
